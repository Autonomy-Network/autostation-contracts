...
const variants = {
  primary: 'bg-gradient-to-r from-primary to-accent text-cyan-800 hover:shadow-md',
...
export const Button: FunctionComponent<ButtonProps> = ({ className = '', onClick, variant = 'primary', disabled = false, children }) => {
  ...
  return(
    <button
      onClick={onClick}
      disabled={disabled}
      className={`${className} px-4 py-2 font-semibold rounded-full shadow-lg ${ disabled ? variants.disabled : variants[variant]}`}
    >
      {children}
    </button>
  );
