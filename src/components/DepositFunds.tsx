  ...
  return (
    <Card className="w-11/12 sm:w-9/12 md:w-1/2 xl:w-1/3 mb-8 relative">
      ...
      {!wallet.state.address ? (
        <>
          <p>
            <button className="underline" onClick={handleConnect}>
              Connect Wallet
            </button>
      ...
      {!!wallet.state.address && state.deposit ? (
        <>
          <span className="flex flex-row space-x-2">
            ...
            <p className="font-extrabold text-transparent bg-clip-text bg-gradient-to-br from-autonomyAcent500 to-autonomySecondary500">
              {state.deposit} {chainCurrency[wallet.state.appNetwork].symbol}
            </p>
          </span>
        </>
      ) : (
      ...
      <div>
        ...
        <div className="flex flex-row items-center space-x-4">
          <Input
            ...
            className="w-1/2 mt-2 h-16 text-center font-semibold"
          >
            0
          </Input>
          <Button onClick={handleDeposit} className="inline-block ml-4 mt-2 border-2 border-white">
            Deposit
          </Button>
          <Button onClick={handleWithdraw} className="inline-block ml-4 mt-2 border-2 border-white">
            Withdraw
          </Button>
        </div>
      </div>

      <p className="text-center mr-1 mt-2 text-sm text-autonomyBlack font-semibold">
        (Only native chain currency allowed currently)
      </p>
    </Card>
  );
};
